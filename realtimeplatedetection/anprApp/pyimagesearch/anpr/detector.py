...
class PyImageSearchANPR:
    ...
    def locate_license_plate_candidates(self, gray, keep=5):
        ...
        rectKern = cv2.getStructuringElement(cv2.MORPH_RECT, (10,4))
    ...
    def build_tesseract_options(self, psm=7):
        ...
        alphanumeric = "ABCDEFGHIJKLMNOPRSTUVYZ0123456789"
...
# 1. Test görüntüsünü yükleyin
image = cv2.imread("test_image.png")

# 2. Görüntüyü yeniden boyutlandırın
resized_image = resize_image(image, height=500)

# 3. Gri tonlamaya dönüştürün
gray = cv2.cvtColor(resized_image, cv2.COLOR_BGR2GRAY)

# 4. PyImageSearchANPR sınıfını başlatın
anpr = PyImageSearchANPR(debug=True)

# 5. Plaka adaylarını bulun
candidates = anpr.locate_license_plate_candidates(gray)

# 6. Plakayı tespit edin
(roi, lpCnt) = anpr.locate_license_plate(gray, candidates)

# 7. Sonuçları gösterin
if roi is not None:
    cv2.imshow("Detected License Plate", roi)
    cv2.waitKey(0)
else:
